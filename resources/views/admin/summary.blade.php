...
<div class="container text-center">
    ...
    <span class="badge rounded-pill p-3 zoom-in mx-3" style="background-color: {{$type->color}}">{{$type->name}}</span>
    @endforeach
    <hr class="py-4">
    <div class="row justify-content-around">
        <div class="col-2 d-flex align-items-center border border-success rounded-pill zoom-in">
            <p class="font-weight-bold m-0">Total orders</p><i class="fas fa-receipt mx-2"></i>
            <strong>{{count($orders)}}</strong>
        </div>
        <div class="col-2 d-flex align-items-center border border-warning rounded-pill zoom-in">
            <p class="font-weight-bold m-0">Total plates</p><i class="fas fa-utensils mx-2"></i>
            <strong>{{count($plates)}}</strong>
        </div>
        <div class="col-2 d-flex align-items-center border border-danger rounded-pill zoom-in">
